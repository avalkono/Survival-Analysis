...
```{r}
full.model <- coxph(Surv(hour, motor) ~ backup + age + bridgecrane + servo + gear + trashrack + slope + elevation, data = hurricane)
...
```{r}
survminer::ggcoxfunctional(coxphmod, data=hurricane)
```
...
data_long %>% arrange(desc(pump))
```

```{r}
data_long = data_long %>% dplyr::select(-c('flood', 'diff'))
```

```{r}
data_long = data_long %>% filter(Hour <= hour)
...
```{r}
surv_object <- Surv(time = data_long$Hour, event = data_long$censor == 1)

full.model <- coxph(surv_object ~ backup + age + bridgecrane + servo + gear + trashrack + slope + elevation, data = data_long)

empty.model <- coxph(surv_object ~ 1, data = data_long)
...
```{r}
coxphmod = coxph(formula = surv_object ~ age + bridgecrane + servo + gear + trashrack + slope + elevation, data = data_long)
summary(coxphmod)
concordance(coxphmod)
```


```{r}
survminer::ggcoxfunctional(coxphmod, data=data_long[, c('survive', 'hour', 'age', 'trashrack', 'slope', 'elevation', 'servo', 'backup', 'Running12Hours')])
```
```{r}
simple_coxphmod <- coxph(surv_object ~ age + trashrack +slope + elevation + servo + backup, data = data_long)
ggcoxfunctional(simple_coxphmod, data = data_long)
```

```{r}
data_long1 = data_long %>% mutate(agebin = case_when(age<7~0, age<9~1, age>=9~2))

data_long1 = data_long1 %>% mutate(slopebin = case_when(slope<4~0, slope<10~1, slope>=10~2))

data_long1 = data_long1 %>% mutate(elevationbin = case_when(slope<3~0, slope<5~1, slope>=5~2))
```

```{r}
coxphmod2 = coxph(formula = surv_object ~ factor(agebin) + trashrack + factor(slopebin) + factor(elevationbin) + servo + backup + Running12Hours, data = data_long1)
summary(coxphmod2)
concordance(coxphmod2)
```

```{r}
hurr.ph.zph <- cox.zph(coxphmod2)
hurr.ph.zph

ggcoxzph(hurr.ph.zph)
```
```{r}
coxphmod3 = coxph(formula = surv_object ~ factor(agebin) + tt(trashrack) + factor(slopebin) + factor(elevationbin) + tt(servo) + tt(backup) + tt(Running12Hours), data = data_long1, tt = function(x, time, ...){x*log(time)})
